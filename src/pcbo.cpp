...
void table_of_ints_init (int max) {
  int i;
  table_of_ints = (str_int *)malloc (sizeof (struct str_int) * max);
  
  for (i = 0; i < max; i++) {
    sprintf(table_of_ints[i].str, "%hu", (unsigned short)i);
...
void read_context (FILE *file) {
  ...
  if (!buff) {
    Rprintf("Cannot allocate buffer, quitting.\n");
    exit (3);
  }
  ...
  if (!context) {
    Rprintf("Cannot allocate bitcontext, quitting.\n");
    exit (5);
  }
...
void print_context_info (void) {
  if (verbosity_level >= 2)
    Rprintf("(:objects %6i :attributes %4i :entries %8i)\n",
            objects, attributes, table_entries);
}

void print_thread_info (int id, struct thread_stat stat) {
  if (verbosity_level >= 2)
    Rprintf("(:proc %3i :closures %7i :computed %7i :queue-length %3i)\n",
...
void print_initial_thread_info (int levels, struct thread_stat stat, int last_level_concepts) {
  if (verbosity_level >= 2)
    Rprintf("(:proc %3i :closures %7i :computed %7i :levels %i :last-level-concepts %i)\n",
  ...
  void *
    ...
    thread_func (void *params) {
      ...
      for (; thread_queue_head[id] < thread_queue[id];) {
        ...
        generate_from_node (thread_intents[id], thread_intents[id] + int_count_a,
                            *(int *) thread_queue_head[id], *(int *) (thread_queue_head[id] + INT_SIZE), id);
...
void parallel_generate_from_node (unsigned long *intent, unsigned long *extent, int start_int, int start_bit,
                                  int rec_level, int id) {
  int i, total;
  static int num = 0;
  size_t supp = 0;
  unsigned long *new_extent;
  ...
  out:
    if (rec_level == 0) {
      ...
      for (; thread_queue_head[id] < thread_queue[id];) {
        memcpy (thread_intents[id], thread_queue_head[id], BYTE_COUNT_A + BYTE_COUNT_O);
        thread_queue_head[id] += BYTE_COUNT_A + BYTE_COUNT_O;
        generate_from_node (thread_intents[id], thread_intents[id] + int_count_a, *(int *) thread_queue_head[id],
...
void find_all_intents (void) {
  ...
  if (verbosity_level >= 3)
    Rprintf("INFO: running in %i threads\n", threads);
  ...
  if (verbosity_level >= 3)
    Rprintf("INFO: total %7i closures\n", counts[0].closures);
  if (verbosity_level >= 3)
    Rprintf("INFO: total %7i concepts\n", counts[0].computed);
}
